...
export function explainVATDecision(input: VATExplanationInput): string {
  ...
  if (isUK) {
    if (category === "Financial Services") {
      allRisks.push(
        "No input VAT reclaim is available on costs directly attributable to exempt financial services supplies.",
      );
    }
    if (category === "Insurance") {
      allRisks.push(
        "Insurance Premium Tax (IPT) applies separately — ensure IPT is correctly calculated and reported.",
      );
    }
    if (category === "Education") {
      allRisks.push(
        "Verify that the education provider meets HMRC eligibility criteria (schools, universities, approved training providers).",
      );
    }
    if (category === "Exports") {
      allRisks.push(
        "Retain proof of export documentation (customs records, shipping evidence) to support the 0% zero-rating claim.",
      );
    }
    if (category === "Intra-EU B2B") {
      allRisks.push(
        "Obtain and verify a valid EU VAT ID from the buyer before applying the reverse charge mechanism.",
      );
    }
  }
  ...
  if (allRisks.length === 0) {
    lines.push(
      "No significant compliance risks detected for this transaction.",
    );
  } else {
