...
export default function Transaction({
  ...
  const [buyerCountry, setBuyerCountry] = useState<CountryCode>(
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Configure your transaction parameters to calculate the applicable VAT
          rate.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-5">
          ...
          <div>
            <label
              htmlFor="buyerCountrySelect"
              className="block text-sm font-medium text-foreground mb-1"
            >
              Buyer Country
            </label>
            <select
              id="buyerCountrySelect"
              value={buyerCountry}
          ...
          <div>
            ...
            <input
              ...
              value={netAmount}
              onChange={(e) =>
                setNetAmount(Number.parseFloat(e.target.value) || 0)
              }
              className="w-full px-3 py-2 bg-background border border-border rounded-md text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              placeholder="1000"
              min="0"
          ...
          {calculationResult && hasCalculated ? (
            ...
            <div className="bg-card border border-border rounded-xl p-6 flex flex-col items-center justify-center text-center min-h-[200px] gap-3">
              ...
              <p className="text-sm text-muted-foreground">
                Select a VAT category and click <strong>Calculate VAT</strong>{" "}
              ...
              {importantNotices.map((notice) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <AlertTriangle
                      className={`w-4 h-4 mt-0.5 flex-shrink-0 ${severityIconStyles[notice.severity]}`}
                    />
                    <div>
